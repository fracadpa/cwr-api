...
export class UsersService {
  ...
  async create(
    ...
    // Transform TenantDto and SubscriptionDto to Tenant and Subscription domain objects
    const tenant: Tenant | undefined = createUserDto.tenant
      ? ({ id: createUserDto.tenant.id } as Tenant)
      : undefined;
    const activeSubscription: Subscription | null | undefined =
      createUserDto.activeSubscription
        ? ({ id: createUserDto.activeSubscription.id } as Subscription)
        : undefined;
  ...
  async remove(id: User['id'], manager?: EntityManager): Promise<void> {
    ...
    await this.usersRepository.remove(id, manager); // Passed manager
  }
}
